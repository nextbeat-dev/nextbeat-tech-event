...
今日はこの言語の **ネイティブコンパイラ** をScala 3（`scala-cli` / `sbt`)で作ります。
...
Claude Code 向けプロンプト集です。ビルドツールには **sbt** を使います。
...
- すべてのソースは `package nblang` 配下。ルート直下に `.scala` を散らかさない。
- `examples/` はプロジェクトルート直下。
...
外部依存ゼロ（`libraryDependencies` 無し）。Scala 標準ライブラリだけで完結する想定。

## ビルド・実行コマンド

```bash
# .nb → .ll 生成（sbt 経由でメインクラスを呼ぶ）
...
sbt 初回起動は重いので、対話モード（`sbt` 単体で起動 → `run examples/sum.nb`）で
何度も叩く方が快適。
...
`Ast.scala` 1 ファイルに集約。`enum` で sum type、`case class` で product type。
...
要件：
- Scala 3.5+、ビルドは sbt（build.sbt + project/build.properties）
- 標準レイアウト：
    nblang/
      ├── build.sbt
      ├── project/build.properties
      └── src/main/scala/nblang/
            ├── Ast.scala / Lexer.scala / Parser.scala
            ├── TypeCheck.scala / CodeGen.scala / Main.scala
  ソースは全部 `package nblang` 配下
- AST は enum + case class で表現
- 外部ライブラリは使わない（Scala 標準のみ、libraryDependencies は空）
- LLVM IR は StringBuilder で組み立てる
- target triple は実行環境を検出して書き分ける（macOS arm64 / Linux x86-64）

最初のゴール：
- examples/sum.nb（1〜10 の和）をコンパイルして examples/sum.ll を生成、
  `sbt 'run examples/sum.nb'` が通ること
...
build.sbt / project/build.properties の中身：
(両方の内容をペースト)

最小構成（依存ゼロ、Scala 3.5+、mainClass = nblang.Main）に直してほしい。
```

### 一気に書きすぎて全体把握できなくなった

```text
今の src/main/scala/nblang/ 配下の各ファイル（Main.scala / Lexer.scala /
Parser.scala / Ast.scala / TypeCheck.scala / CodeGen.scala）の役割を
1 行ずつ要約してほしい。
...
- 進捗ログが stdout に流れる場合があるので、IR は `Files.writeString` でファイル直書き、ログは `Console.err.println()` に
- sbt 対話モードを起動して `~run examples/sum.nb` で監視実行も可能
- `target/` `project/target/` `project/project/` は `.gitignore` に入れておく
