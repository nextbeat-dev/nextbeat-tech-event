...
ゴールは正規表現理論の講義ではなく、**「基本設計さえ握れば、AI（Claude Code / Codex）で“使えるもの”がぱっと立ち上がってしまう」という開発体験**を持ち帰ること。
...
## 当日の流れ（イベント全体 19:00-21:00 / ハンズオン枠 70分）

受付・自己紹介（19:00-19:10）→ イントロ＆ReDoS実演（19:10-19:20、講師デモ）のあと、
ハンズオン本体（19:20-20:30）は次の6ステップ。時刻はハンズオン開始からの相対表記。

| 開始 | 所要 | ステップ | やること |
|---|---|---|---|
| +0分 | 5分 | **0. 同期チェック** | 全員 `start` ブランチで `pnpm test` が赤22件になっているか確認 |
| +5分 | 15分 | **1. 設計を握る** | 正規表現を「状態」とみなす＝微分。`nullable`/`derivative` の規則表を紙で合意（[SPEC.md](./SPEC.md)。`∂b(a?b)` の手トレースまで） |
| +20分 | 15分 | **2. 山場①** | 設計表を AI（Claude Code / Codex）に投入（[AGENT_PROMPTS.md](./AGENT_PROMPTS.md) ①）→ コアが数分で生成 → `pnpm test` で赤が一斉に緑 |
| +35分 | 8分 | **3. まず壊す** | 正規化なしのまま `pnpm bench:redos 16` → 自作エンジンでも状態が爆発して打ち切られるのを体感（**n は 16 まで。20 以上・引数省略は禁止**） |
| +43分 | 12分 | **4. 正規化②** | ACI 正規化を投入（AGENT_PROMPTS ②）→ `pnpm test:all` 全緑 → `pnpm bench:linear` で 1000万文字・状態数一定＝O(n) を実証 |
| +55分 | 8分 | **5. 実食** | 持参した「一番複雑な正規表現」を対応構文に簡約して `pnpm cli` に食わせる |
| +63分 | 7分 | **6. 設計の境界** | 後方参照 `\1` を**あえて入れない**理由＝線形保証（[SPEC.md](./SPEC.md) 末尾）→ 締め |
